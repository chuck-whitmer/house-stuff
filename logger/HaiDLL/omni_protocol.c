...
HAIEXPORT int omni_put_name(hai_comm_id *id,  int type, int num, const char *name)
{
    ...
    if ((type == NAMES_ZONE) || (type == NAMES_MESSAGE))
        len = 15;
    else
        len = 12;
    ...
    if (num < 256)
    {
        ...
        strncpy(req.data.ns.text, name, len);
    ...
    else
    {
        ...
        strncpy(req.data.nl.text, name, len);
